...
namespace XboxSampleFramework
{
    struct Help::CalloutBox
    {
        ...
        void Render( _In_ XSF::Draw& pDraw, _In_ D3DDeviceContext* pContext, XMFLOAT2 vScaling, _In_opt_ const D3D11_VIEWPORT* pViewport = nullptr, BOOL fastSemanticsEnabled = FALSE );
    };
}
...
void XSF::Help::CalloutBox::Render( XSF::Draw& draw, D3DDeviceContext* pContext, XMFLOAT2 vScaling, const D3D11_VIEWPORT* pViewport, BOOL fastSemanticsEnabled )
{
    D3D11_RECT window = pFont->GetWindow();
    ...
    pFont->Begin( pContext, pViewport, fastSemanticsEnabled );
    pFont->DrawText( vScaling.x * (FLOAT) boundsContent.left, vScaling.y * (FLOAT) boundsContent.top,
        foreground, text, 0, vScaling.x * (FLOAT) contentSize.cx );
    pFont->End();
...
//----------------------------------------------------------------------------------------------------------------------
void XSF::Help::CalloutBox::Create( _Out_ Help::CalloutBox& dest, _In_z_ LPCWSTR pText, _In_ XSF::BitmapFont* pFont,
                                      COLORREF color, HelpID helpSlot )
{
    memcpy( &dest, &CalloutTemplates[ (size_t)helpSlot ], sizeof( CalloutBox ) );
    dest.text = pText;
...
{
    XSFScopedNamedEventFunc( NULL, 0 );
...
//----------------------------------------------------------------------------------------------------------------------
_Use_decl_annotations_
void XSF::Help::Render( D3DDeviceContext* pCtx, const D3D11_VIEWPORT *pViewport, BOOL fastSemanticsEnabled )
{
    XSF_ASSERT( m_IsVisible && "Only call if IsVisible is TRUE" );

    XSFScopedNamedEventFunc( pCtx, 0 );
    ...
    stockStates.ApplyBlendState( pCtx, XSF::StockBlendStates::AlphaBlend );

    pCtx->HSSetShader( nullptr, nullptr, 0 );
    pCtx->DSSetShader( nullptr, nullptr, 0 );
    pCtx->GSSetShader( nullptr, nullptr, 0 );

    m_pCanvas->Begin( pCtx, nullptr, pViewport );

    ID3D11SamplerState* pSamplerState = 
        XSF::StockRenderStates::GetStates().GetSamplerState( StockSamplerStates::MinMagLinearMipPointUVWClamp );

    // Render the background texture.
    m_pCanvas->TexturedQuad( m_pBackground, pSamplerState );

    // Render the call-outs in reverse order.
    for( int i = m_CalloutCount - 1; i >= 0; --i )
    {
        m_pCalloutBoxes[i].Render( *m_pCanvas, pCtx, m_DisplayScaleFactor, pViewport, fastSemanticsEnabled );
    }
...
//----------------------------------------------------------------------------------------------------------------------
void XSF::Help::Initialize( D3DDevice* pDevice, UINT frameWidth, UINT frameHeight )
{
    XSF_ASSERT( !m_IsInitialized && "Already initialized...");

    m_DisplayBounds.cx = (LONG)frameWidth;
    m_DisplayBounds.cy = (LONG)frameHeight;
    
    // Load the background texture...
    XSF_ERROR_IF_FAILED( XSF::CreateTextureFromFile( pDevice, BACKGROUND_IMAGE_FILENAME, &m_pBackground ) );
    ...
    XSF_ERROR_IF_FAILED( m_pCanvas->Initialize( pDevice ) );
...
//----------------------------------------------------------------------------------------------------------------------
void XSF::Help::Initialize( const SampleFramework& sampleFramework )
{
    const SampleFramework::Settings& settings = sampleFramework.SampleSettings();
    Initialize( sampleFramework.GetDevice(), settings.m_frameBufferWidth, settings.m_frameBufferHeight );
}
...
//----------------------------------------------------------------------------------------------------------------------
void XSF::Help::LoadFonts( _In_ D3DDevice* pDevice )
{
    XSF_ASSERT( !ms_DefaultTitleFont && !ms_DefaultCalloutFont &&
        !ms_DefaultDescriptionFont && "Fonts already loaded?");

    ms_DefaultCalloutFont = new BitmapFont();
    ms_DefaultTitleFont = new BitmapFont();
    ms_DefaultDescriptionFont = new BitmapFont();

    const D3D11_RECT rc = { 0, 0, m_DisplayBounds.cx, m_DisplayBounds.cy };
    XSF_ERROR_IF_FAILED( ms_DefaultCalloutFont->Create( pDevice, ARIAL_16_FONT, &rc ) );
    XSF_ERROR_IF_FAILED( ms_DefaultDescriptionFont->Create( pDevice, ARIAL_12_FONT, &rc ) );
    XSF_ERROR_IF_FAILED( ms_DefaultTitleFont->Create( pDevice, ARIAL_20_FONT, &rc ) );
}
...
void XSF::Help::Shutdown()
{
    if( m_IsInitialized )
    {
        DestroyCallouts();
        DestroyFonts();
        XSF_SAFE_RELEASE( m_pBackground );
...
//----------------------------------------------------------------------------------------------------------------------
void XSF::Help::SetHelpText( _In_z_ LPCWSTR pTitle, _In_z_ LPCWSTR pDescription,
...
void XSF::Help::BuildHelpScreen( const HelpScreen& helpText )
{
    ...
    if( helpText.titleText != nullptr )
    {
        CalloutBox::Create( m_pCalloutBoxes[ index++ ], helpText.titleText, pTitleFont, helpText.titleColor,
            XSF::HelpID::TITLE_TEXT );
    }

    if( helpText.descriptionText != nullptr )
    {
        CalloutBox::Create( m_pCalloutBoxes[ index++ ], helpText.descriptionText, pDescriptionFont,
            helpText.descriptionColor, XSF::HelpID::DESCRIPTION_TEXT );
    }

    for( int i = 0; i < helpText.buttonCount; ++i )
    {
        CalloutBox::Create( m_pCalloutBoxes[ index++ ], helpText.buttonAssignments[i].buttonText,
            pCalloutFont, helpText.calloutColor, helpText.buttonAssignments[i].id );
    }
...
void XSF::Help::MeasureCallouts()
{
    for( int i = 0; i < m_CalloutCount; ++i )
    {
        ...
        INT pixelWidth = (INT)ceil( fWidth );
        INT pixelHeight = (INT)ceil( fHeight );
...
{
    m_DisplayScaleFactor.x	 = (FLOAT)m_DisplayBounds.cx / (FLOAT)DESIGN_SURFACE_WIDTH;
    m_DisplayScaleFactor.y	 = (FLOAT)m_DisplayBounds.cy / (FLOAT)DESIGN_SURFACE_HEIGHT;
}
